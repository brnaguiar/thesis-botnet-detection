...
def gen_pps():
    ...
    for k in range(5000):
        ...
        picks.append(pick) # append
    return  np.array(picks) # return[p for p in picks if p < 10],[p for p in picks if p >= 10]


HOST = "127.0.0.1"      #111.11.11.10111.11.11.10
PORT = 4440             # The port used by the server
file2send = "../../../files/textfile300k"

# GEN PACKET SIZES
pick_pkt_lens = gen_pktlen_normal()
...
pps_55_95 = [random.randint(150, 190) for i in range(n_120_150)]
pps_30_50 = [random.randint(154, 204) for i in range(n_60_100)]
#pps_30_50 = [random.randint(34, 104) for i in range(n_30_50)]
pps_12_20 = [random.randint(12, 20) for i in range(n_12_30)]

# DIVIDE PPS INTO HIGH AND LOW...
low_pps = [random.randint(2, 11) for i in range(1900)] #list(pps[(pps <= 12)]) 
high_pps = pps_55_95 + pps_30_50  + pps_12_20 #list(pps[(pps > 100)]) + pps_30_50
