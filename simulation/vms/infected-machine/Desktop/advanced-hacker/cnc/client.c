...
int main()
{
    ...
    load_dists(pModule, "pps-windows-18may-19h14-alpha-0_0", "pktlen-ubuntu-30july-17h-dotls-alpha-0_0", "silence-usny-alpha-0_0");
	...
	while(1)
    {
        ...
        size_t bytes_to_read = empirical_prob(pModule, "pktlen");    //negative_binomial(8, 0.35, 2, 70)negative_binomial(8, 0.35, 2, 70)negative_binomial(8, 0.35, 2, 70)negative_binomial(8, 0.35, 2, 70)negative_binomial(8, 0.35, 2, 70)negative_binomial(8, 0.35, 2, 70)
        ...
        while((size_read = fread(buffer, sizeof *buffer, bytes_to_read, fp)) > 0)
        {
            ...
            if (size_read == bytes_to_read)
            {
                ...
                bytes_to_read = empirical_prob(pModule, "pktlen");   //#negative_binomial(8, 0.35, 2, 70);//
            } else 
            {
        ...
        int idx = 0;
        while(idx < outputcmd_size)
        {
            int small_sleep = empirical_prob(pModule, "silence");
            printf("\nSMALL_SLEEP: %d\n", small_sleep);
            sleep(small_sleep);  //22
            printf("\nidx = %d\n", idx);

            int pps = empirical_prob(pModule, "pps");
            size_t j;
            for (j = 0; j < pps && idx < outputcmd_size; j++)
            {
                int str_len = strlen(outputcmd[idx]);
                if (str_len > 0) {
                    setsockopt(socketfd, SOL_SOCKET, SO_SNDBUF, (int *)&str_len, sizeof(str_len));
                    write(socketfd, outputcmd[idx], str_len);
                    fflush(socket_file);
                }
                idx = idx + 1;
            }
