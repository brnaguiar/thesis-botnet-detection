...
int main()
{
    ...
    FILE *file_ptr = fopen("../../../files/textfile300k", "rb");  //
    int eof_flag = 1;
    ...
    load_dists(pModule, "pps-ubuntu-30july-03h-dotls-adblock-alpha-0_0", "pktlen-ubuntu-30july-03h-dotls-adblock-alpha-0_0", "silence-ubuntu-30july-03h-dotls-adblock-alpha-0_2");
...
//                    setsockopt(socket_desc, SOL_SOCKET, SO_RCVBUF, (int *)&str_len, sizeof(str_len));
                ...
                //printf("\nLEN: %d | FD : %d | MESSAGE: %s\n", str_len, socket_desc, data_array[i][j]);  //�ñ�
