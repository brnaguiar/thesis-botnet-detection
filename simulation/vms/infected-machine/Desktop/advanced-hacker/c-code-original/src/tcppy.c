...
void gen_data2send(char ***data_array, int *eof_flag, FILE *file_ptr, int *size_pps, int number_of_pps, int *real_npps)
{
        ...
        for (i = 0; i < number_of_pps && *eof_flag; i++) 
        {
            ...
            fp = popen("python3 src/stats/distributions.py -m beta_negative_binomial -p 1.5 1.7 3", "r");  
            ...
            for (j = 0; j < pps && *eof_flag; j++)
            {
                ...
                fp = popen("python3 src/stats/distributions.py -m negative_binomial -p 8 0.35", "r");
                fgets(input, 5, fp); int status = pclose(fp);
